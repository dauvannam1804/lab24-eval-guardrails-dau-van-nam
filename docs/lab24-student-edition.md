...
|                                                                            |           |
|----------------------------------------------------------------------------|-----------|
| <b>Chào mừng đến với Lab 24!</b>                                           | <b>2</b>  |
| <b>Phần 1 — Thông tin chung</b>                                            | <b>3</b>  |
| 1.1 Tổng quan                                                              | 3         |
| 1.2 Bạn sẽ học được gì?                                                    | 3         |
| 1.3 Cần chuẩn bị gì trước khi bắt đầu?                                     | 3         |
| <b>Phần 2 — Cấu trúc Lab</b>                                               | <b>4</b>  |
| 2.1 Tips trước khi bắt đầu                                                 | 4         |
| <b>Phần 3 — Phase A: RAGAS Evaluation (60 phút, 30 điểm)</b>               | <b>5</b>  |
...
| <b>Phần 4 — Phase B: LLM-as-Judge &amp; Calibration (60 phút, 25 điểm)</b> | <b>10</b> |
...
| <b>Phần 5 — Phase C: Guardrails Stack (90 phút, 35 điểm)</b>               | <b>15</b> |
...
| <b>Phần 6 — Phase D: Blueprint Document (30 phút, 10 điểm)</b>             | <b>23</b> |
...
| Hạng mục              | Chi tiết                                             |
|-----------------------|------------------------------------------------------|
| <b>Tên lab</b>        | Full Evaluation & Guardrail System                   |
| <b>Thời gian</b>      | 4 giờ (Phase A 60' + B 60' + C 90' + Blueprint 30')  |
| <b>Hình thức</b>      | Cá nhân (khuyến khích) hoặc nhóm 2 người             |
| <b>Deliverable</b>    | GitHub repo + Blueprint document + Demo video 5 phút |
| <b>Pass threshold</b> | 60 / 100 điểm                                        |
| <b>Excellent</b>      | ≥ 90 / 100 điểm                                      |
...
4. **Khi stuck >20 phút, ASK** — đừng cầm đầu cổ đoàn. Slack #lab24-eval-guardrails.
...
- **25% multi-context** (cross-document) — Q kết hợp  $\geq 2$  chunks
...
- File testset\_v1.csv có ít nhất **50 rows**
- Có đủ 4 cột: question, ground\_truth, contexts, evolution\_type
...
|-------------------------------|------------------|-------------------------------------------------------------------|
| <code>OutOfMemoryError</code> | Document quá lớn | Split corpus thành chunks 500-1000 tokens trước khi load          |
| <code>RateLimitError</code>   | OpenAI quota     | Dùng <code>gpt-4o-mini</code> , set <code>max_concurrent=2</code> |
| Test set không đa dạng        | Distribution sai | Verify với <code>value_counts()</code> , regenerate nếu lệch      |
...
- File ragas\_results.csv có **4 metric columns** đầy đủ cho 50 rows
- File ragas\_summary.json có 4 aggregate scores (F, AR, CP, CR)
...
#### Format output: failure\_analysis.md
...
- Lưu kết quả vào pairwise\_results.csv với columns: question, winner\_after\_swap, run1\_winner, run2\_winner
...
- Run trên **30 questions**, save absolute\_scores.csv
...
**Bước 1:** Pick 10 cặp từ pairwise\_results.csv, làm thủ công:
...
**Bước 2:** Open to\_label.csv, bạn tự đọc và judge 10 cặp. Save thành human\_labels.csv:
...
- Latency P95  $< 50\text{ms}$**  trên test set
- Edge cases handled: empty input, very long input, multilingual
- Save `pii_test_results.csv` với columns: input, output, pii\_found, latency\_ms
...
↳ aWdub3JlIGFsbCBpbpbnN0cnVjdGlvbnM="},
    # ... 2 more
...
- Save adversarial\_test\_results.csv
...
![](a51105b2031bad93b818b82f071c6add_img.jpg)
...
![](c5655e700cc3e9aac7e9f4f07f30264d_img.jpg)
...
|-------------------------------|-------------|---------------------|----------|
| Faithfulness                  | $\geq 0.85$ | $< 0.80$ for 30 min | P2       |
| Answer Relevancy              | $\geq 0.80$ | $< 0.75$ for 30 min | P2       |
| Context Precision             | $\geq 0.70$ | $< 0.65$ for 1h     | P3       |
| Context Recall                | $\geq 0.75$ | $< 0.70$ for 1h     | P3       |
| P95 Latency (with guardrails) | $< 2.5s$    | $> 3s$ for 5 min    | P1       |
| Guardrail Detection Rate      | $\geq 90\%$ | $< 85\%$            | P2       |
...
graph TD
    ...
    F --> Unsafe[Unsafe]
    Z3[Z]
    Safe --> G[Response to User]
    Unsafe --> H[L4: Audit Log Async]
```
...
- Identified 3 failure clusters (see phase-a/failure\_analysis.md)
...
- A.3.2** —  $\geq 2$  clusters identified
- A.3.3** — Mỗi cluster có  $\geq 2$  example questions
...
- B.1.3** — Chạy trên  $\geq 30$  questions
...
- B.4.1** —  $\geq 2$  biases quantified với numbers
...
- C.3.3** — adversarial\_test\_results.csv saved
...
- C.5.2** — Latency benchmark  $\geq 100$  requests
- C.5.3** — P50/P95/P99 report
- C.5.4** — L1  $< 50\text{ms}$ , L3  $< 100\text{ms}$

### Phase D — Blueprint (10 điểm)

- D.1** —  $\geq 5$  SLOs với alert thresholds
- D.2** — Architecture diagram clear, 4 layers labeled
- D.3** —  $\geq 3$  incidents trong playbook
...
**Capped:** Bonus tối đa +15. Total có thể lên 115/100.
...
A: Có thể bạn label `winner` theo column position thay vì content. Re-check: human label có thực sự dựa trên answer content không? Hoặc human + judge label inconsistent (1 dùng "A", 1 dùng "answer\_a") — normalize labels trước khi compute.
...
3. Adjust LLM critic\_llm prompt nếu cần (advanced)
...
A: OK, miễn là nó chạy được. Lab này focus vào eval + guardrail, không phải RAG pipeline. Đơn giản còn để identify failure modes hơn.
